"""DeepSeek-OCR2 image processor (adapted from official process/image_process.py).

Changes from official:
- Config imports replaced with constants
- TOKENIZER is a required constructor parameter (no module-level loading)
- tokenize_with_images() accepts prompt as parameter
- Removed AutoProcessor.register at module level
"""
...
from deepseek_vllm.constants import IMAGE_SIZE, BASE_SIZE, MIN_CROPS, MAX_CROPS
...
class DeepseekOCR2Processor(ProcessorMixin):
    ...
    def tokenize_with_images(
        ...
        Args:
            ...
            prompt: Full prompt string containing <image> placeholder
            bos: Add BOS token
            eos: Add EOS token
            cropping: Enable dynamic image tiling
        """
        conversation = prompt
        ...
        assert len(tokenized_str) == len(
            images_seq_mask), f"tokenize_with_images: tokenized_str length {len(tokenized_str)} != images_seq_mask length {len(images_seq_mask)}"
