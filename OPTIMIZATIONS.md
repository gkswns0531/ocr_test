...
| batched_tokens | Activation | KV Cache | Req/s | Tok/s | p50 | p95 |
|----------------|-----------|----------|-------|-------|-----|-----|
| 32,768 (32K) | ~16 GB | 155.2 GB | 22.0 | 3150 | 5.7s | 20.2s |
| **65,536 (64K)** | **~21 GB** | **150.6 GB** | **24.6** | **3611** | **5.4s** | **18.1s** |
| 131,072 (128K) | ~29 GB | 142.2 GB | 22.4 | 3243 | 5.3s | 19.7s |
| 262,144 (256K) | ~46 GB | 125.3 GB | 15.9 | 2350 | 7.5s | 13.0s |
| 524,288 (512K) | ~80 GB | 90.8 GB | 14.9 | 2138 | 7.8s | 14.0s |

조건: B200, FP8, 512 요청, 256 동시, 16 warmup, 서버 매회 완전 재시작

**결론: 64K가 최적, 128K 대비 +10% throughput.**

- **32K**: 스텝당 처리량 부족 → 스케줄링 오버헤드 증가
- **64K**: 모델 크기(1.36GB)에 맞는 sweet spot. activation 최소화 + 충분한 스텝 처리량
- **128K+**: activation 낭비, KV cache 줄어서 prefix cache hit rate 하락

> **65,536이 GLM-OCR (~1.3B FP8)의 최적점. 모델 크기에 맞게
> activation과 KV cache 간 최적 균형을 찾아야 함.**

### 서버 설정 변경 이력

| 설정 | 초기 | 현재 | 변경 이유 |
|------|------|------|-----------|
| `max-num-batched-tokens` | 131072 | **65536** | A/B 테스트(32K~524K) 결과 64K가 최적 |
